...
export default function AiCopilotPage() {
  ...
  const cardClass =
    "bg-card rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.04),0_0_0_1px_rgba(0,0,0,0.02)]";
    ...
    <div className="flex flex-col gap-6 h-full max-w-[960px]">
      ...
      <div className={`${cardClass} p-6`}>
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 bg-background border border-border/50 rounded-[10px] px-4 py-2.5 text-[13px] text-foreground placeholder:text-muted-foreground transition-colors hover:border-border focus:border-primary"
          />
          <button
            onClick={() => query && runQuery(query)}
            className="flex items-center gap-2 px-5 py-2.5 text-[12px] bg-primary text-primary-foreground rounded-[10px] hover:bg-primary/90 disabled:opacity-50 transition-colors shadow-sm"
      ...
      {result && !loading && (
        <div className="space-y-4 flex-1 overflow-auto">
          {/* Context bar */}
          <div className="flex items-center gap-2 text-[11px] text-muted-foreground bg-violet-500/[0.06] border border-violet-500/10 rounded-[10px] px-4 py-2.5">
          ...
          <div className={`${cardClass} p-6`}>
            ...
            <div className="flex flex-wrap gap-2">
              {result.relatedTxns.map((t) => (
                <span
                  key={t}
                  className="px-2.5 py-1.5 text-[11px] bg-primary/[0.06] text-primary rounded-[8px] cursor-pointer hover:bg-primary/[0.12] transition-colors font-medium"
                >
                  {t}
                </span>
              ))}
